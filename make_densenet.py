...
def conv_bn_relu(bottom, ks, nout, stride, pad, dropout):
...
def add_layer(bottom, num_filter, dropout):
    ...
    newC = conv_bn_relu(bottom, ks=3, nout=num_filter, stride=1, pad=1, dropout=dropout)
    concate = L.Concat(bottom, newC, axis=1)
    return concate
...
def densenet(data_file, mode='train', batch_size=64, depth=40, first_output=16, growth_rate=12, dropout=0.2):
    ...
    model = conv_bn_relu(data, ks=3, nout=nchannels, stride=1, pad=1, dropout=dropout)
...
def make_net():
    ...
    with open('train_densenet.prototxt', 'w') as f:
        ...
        print(str(densenet('/home/tl486/git_TCcaffe/caffe2/caffe/examples/cifar10/cifar10_train_lmdb', batch_size=64)), file=f)

    with open('test_densenet.prototxt', 'w') as f:
        print(str(densenet('/home/tl486/git_TCcaffe/caffe2/caffe/examples/cifar10/cifar10_test_lmdb', batch_size=50)), file=f)
